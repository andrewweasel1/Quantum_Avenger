...
**P0 — Enabler (the keystone).** `add_cross_sectional_factors(frame)` does `group_by("date")` then ranks / z‑scores / sector‑neutralizes across the universe (universe from `StaticUniverseProvider`); called in `build_training_frame`; names appended to `FEATURE_COLS` behind `features.factor_set`. Adds the per‑date axis without disturbing per‑ticker features. **Done when:** a momentum/reversal/low‑vol/seasonality set is golden‑pinned and the offline pipeline shows the causal selector + DSR gates consuming and deflating it.
