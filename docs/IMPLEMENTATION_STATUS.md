...
2. **Information bars + fractional differentiation** (§A) — min‑`d` frac‑diff for stationary‑with‑memory features; dollar/volume/imbalance bars upstream. **◐ frac‑diff (+ vol/microstructure families) shipped** — `features/{fracdiff,vol_estimators,microstructure,extended}.py` behind `features.extended_features` (default off), golden‑pinned; frac‑diff passes the ADF stationarity assertion (log‑price −1.35 → −3.42). **Pending:** information bars (need intraday/tick data — a bar resampler over a tick source); GARCH volatility.
