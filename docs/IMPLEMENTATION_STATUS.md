...
4. **Stat‑arb: cointegration / Ornstein‑Uhlenbeck** (§C) — a market‑neutral second strategy family. **✅ shipped** — `tournament/stat_arb.py` (self‑contained ADF, Engle‑Granger pairs, OU half‑life, causal mean‑reversion rule, `find_cointegrated_pairs`) + `pipeline._run_stat_arb` (within‑sector pairs → date‑indexed sleeves → stat‑arb book via the P4 layer → `stat_arb.json`), behind `stat_arb.enabled`. **Pending:** Johansen multivariate baskets; running the stat‑arb sleeves through the full DSR/PBO promotion gauntlet (today they report Sharpe + feed the book).
5. **Portfolio layer: HRP/NCO + RMT/Ledoit‑Wolf denoising** (§H,§F) — combine sleeves into target weights on a denoised covariance. **✅ shipped** — `portfolio/` package: `denoise_covariance` (RMT), `ledoit_wolf_covariance` (sklearn), `hrp_weights`/`inverse_variance_weights`, `combine_returns`; `run_offline_pipeline` writes a per‑sector book to `portfolio.json` (behind `portfolio.enabled`). **Pending:** NCO + IC‑weighting; calendar‑aligned sleeve streams (the current cross‑sector book truncates to a common length — exact alignment arrives with date‑indexed sleeves).
6. **Data‑snooping guards: White's Reality Check / Hansen's SPA** (§J) — multiple‑*strategy* bootstrap tests complementing per‑strategy DSR/PBO. **✅ shipped** — `evaluation/reality_check.py` (`whites_reality_check`, `hansens_spa`; shared stationary block bootstrap), recorded per sector over the grid‑search trials behind `evaluation.reality_check_enabled`; golden‑pinned (noise ≈ 0.7, a skilled best‑of‑20 ≈ 0.003).
