...
5. **Portfolio layer: HRP/NCO + RMT/Ledoit‑Wolf denoising** (§H,§F) — combine sleeves into target weights on a denoised covariance. **✅ shipped** — `portfolio/` package: `denoise_covariance` (RMT), `ledoit_wolf_covariance` (sklearn), `hrp_weights`/`inverse_variance_weights`, `combine_returns`; `run_offline_pipeline` writes a per‑sector risk allocation to `portfolio.json` (behind `portfolio.enabled`). The **stat‑arb** book is now exact (its OOS sleeves are date‑indexed and HRP‑combined); the **cross‑sector champion** book uses honest inverse‑variance weighting (the champions are per‑sample ticker‑stacked, not calendar‑aligned). **Pending:** NCO + IC‑weighting; date‑indexing the per‑sector champion sleeves for an exact cross‑sector HRP book.
